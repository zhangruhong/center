...
/*    @PostConstruct
    ...
    public void validate(MultipartFile file) {  
        ...
        if (allowedContentTypes!=null && allowedContentTypes.length>0 && !ArrayUtils.contains(allowedContentTypes, file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1).toLowerCase()))  
        	throw new FileException("您上传的文件类型"+file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")+1)+"不符合要求，可上传的文件类型为"+ArrayUtils.toString(allowedContentTypes));  
        if (file.getSize() > maxSize)  
            throw new FileException("您的文件大小为"+(file.getSize()/1024/1024)+"M超过可上传的最大值"+maxSize/1024/1024+"M");  
    }
